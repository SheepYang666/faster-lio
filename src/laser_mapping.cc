...
bool LaserMapping::LoadParams() {
    ...
    this->declare_parameter<double>("esti_plane_threshold", 0.1);
    ...
    this->declare_parameter<double>("preprocess.time_scale", 1e-3);
    ...
    this->declare_parameter<double>("ivox_grid_resolution", 0.2);
    ...
    this->get_parameter("esti_plane_threshold", esti_plane_threshold);
    options::ESTI_PLANE_THRESHOLD = esti_plane_threshold;
    ...
    double det_range_d;
    this->get_parameter("mapping.det_range", det_range_d);
    det_range_ = static_cast<float>(det_range_d);
    ...
    double blind_val;
    this->get_parameter("preprocess.blind", blind_val);
    preprocess_->Blind() = blind_val;
    double time_scale_val;
    this->get_parameter("preprocess.time_scale", time_scale_val);
    preprocess_->TimeScale() = static_cast<float>(time_scale_val);
    this->get_parameter("preprocess.lidar_type", lidar_type);
    int scan_line;
    this->get_parameter("preprocess.scan_line", scan_line);
    preprocess_->NumScans() = scan_line;
    int point_filter_num;
    this->get_parameter("point_filter_num", point_filter_num);
    preprocess_->PointFilterNum() = point_filter_num;
    bool feature_enabled;
    this->get_parameter("feature_extract_enable", feature_enabled);
    preprocess_->FeatureEnabled() = feature_enabled;
    ...
    this->get_parameter("ivox_grid_resolution", ivox_resolution);
    ivox_options_.resolution_ = static_cast<float>(ivox_resolution);
...
void LaserMapping::IMUCallBack(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    if (abs(timediff_lidar_wrt_imu_) > 0.1 && time_sync_en_) {
        msg->header.stamp = common::toROSTime(timediff_lidar_wrt_imu_ + common::toSec(msg_in->header.stamp));
    }
...
void LaserMapping::PublishPath() {
    ...
    msg_body_pose_.header.stamp = common::toROSTime(lidar_end_time_);
...
void LaserMapping::PublishOdometry() {
    ...
    odom_aft_mapped_.header.stamp = common::toROSTime(lidar_end_time_);
    ...
    t.header = odom_aft_mapped_.header;
    t.child_frame_id = tf_imu_frame_;
    ...
    t.transform.rotation = odom_aft_mapped_.pose.pose.orientation;
...
void LaserMapping::PublishFrameWorld() {
    ...
    if (run_in_offline_ == false && scan_pub_en_) {
        ...
        laserCloudmsg.header.stamp = common::toROSTime(lidar_end_time_);
